...
final public class Game {
    ...
    private static int aStarPathFinding(int startX, int startY, int goalX, int goalY) {
        ...
        public static void syncCrewCards() {
        for (Player p : players){
            ArrayList<CrewCard> portCards = p.getHomePort().getCards();
            for (int i = 0; i < portCards.size(); i++){
                CrewCard card = portCards.get(i);
                p.addCard(card);
                p.getHomePort().removeCard(card);
            }
