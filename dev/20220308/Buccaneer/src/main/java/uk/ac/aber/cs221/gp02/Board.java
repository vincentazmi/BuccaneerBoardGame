...
public class Board {
    ...
    //for debugging
    public void printOnPath(){
        for (int y = 0; y < 20; y++) {
            for (int x = 0; x < 20; x++) {
                if(tiles[x][y].isPath()){
                    System.out.println(x + " " + y + " " + tiles[x][y].getXCor() + " " + tiles[x][y].getYCor());
                }
    ...
    public void markReachable(Player p, State state) {
        cleanTiles();
        ...
        if (state == State.ROTATE) {
            tiles[x-1][y-1].setPath(true);
            tiles[x-1][y].setPath(true);
            tiles[x-1][y+1].setPath(true);
            tiles[x+1][y-1].setPath(true);
            tiles[x+1][y].setPath(true);
            tiles[x+1][y+1].setPath(true);
            tiles[x][y-1].setPath(true);
            tiles[x][y+1].setPath(true);
        }
        if (state == State.MOVE) {
            switch (dir) {
                case NORTH -> {
                    for (int i = 1; i <= steps; i++) {
                        if (y - i < 0 || tiles[x][y - i].getElement() instanceof Island)
                            break;
                        tiles[x][y - i].setPath(true);
                    }
                }
                case NORTHEAST -> {
                    for (int i = 1; i <= steps; i++) {
                        if (y - i < 0 || x + i > 19 || tiles[x + i][y - i].getElement() instanceof Island)
                            break;
                        tiles[x + i][y - i].setPath(true);
                    }
                }
                case EAST -> {
                    for (int i = 1; i <= steps; i++) {
                        if (x + i > 19 || tiles[x + i][y].getElement() instanceof Island)
                            break;
                        tiles[x + i][y].setPath(true);
                    }
                }
                case SOUTHEAST -> {
                    for (int i = 1; i <= steps; i++) {
                        if (y + i > 19 || x + i > 19 || tiles[x + i][y + i].getElement() instanceof Island)
                            break;
                        tiles[x + i][y + i].setPath(true);
                    }
                }
                case SOUTH -> {
                    for (int i = 1; i <= steps; i++) {
                        if (y + i > 19 || tiles[x][y + i].getElement() instanceof Island)
                            break;
                        tiles[x][y + i].setPath(true);
                    }
                }
                case SOUTHWEST -> {
                    for (int i = 1; i <= steps; i++) {
                        if (y + i > 19 || x - i < 0 || tiles[x - i][y + i].getElement() instanceof Island)
                            break;
                        tiles[x - i][y + i].setPath(true);
                    }
                }
                case WEST -> {
                    for (int i = 1; i <= steps; i++) {
                        if (x - i < 0 || tiles[x - i][y].getElement() instanceof Island)
                            break;
                        tiles[x - i][y].setPath(true);
                    }
                }
                case NORTHWEST -> {
                    for (int i = 1; i <= steps; i++) {
                        if (y - i < 0 || x - i < 0 || tiles[x - i][y - i].getElement() instanceof Island)
                            break;
                        tiles[x - i][y - i].setPath(true);
                    }
                }
